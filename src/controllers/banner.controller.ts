...
export default {
  create: async (req: IReqUser, res: Response) => {
    try {
      await bannerDTO.validate(req.body, { abortEarly: false })
  ...
  findOne: async (req: IReqUser, res: Response) => {
    try {
      const { id } = req.params
      if (!isValidObjectId(id)) return response.notFound(res, "failed to find a banner")
      const result = await BannerModel.findById(id)
      if (!result) return response.notFound(res, "failed to find a banner")
